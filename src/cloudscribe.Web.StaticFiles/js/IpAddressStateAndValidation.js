...
$(document).on('focusout', '.ip-address-input', function () {
    ...
    const $input = $(this);
    const safeId = $input.data('safeid');
    const $errorMsg = $('#errorMessage-' + safeId);

    if (ipv4.test($input.val()) || ipv6.test($input.val())) {
...
$(document).ready(function () {
    ...
    var hasValue = /\S/.test(value);

    $('#searchButton').prop('disabled', !hasValue);
    $('#RemoveSearch').prop('disabled', !hasValue)

    $('#searchTerm').on('input', function () {
        var value = $(this).val();

        $('#searchButton').prop('disabled', !/\S/.test(value));
        $('#RemoveSearch').prop('disabled', !/\S/.test(value));
    });
});