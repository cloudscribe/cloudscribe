...
namespace cloudscribe.Core.Repositories.pgsql
{
    public sealed class UserRepository : IUserRepository
    {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            ISiteUser user = await Fetch(siteId, userId, cancellationToken);
            if (user != null)
            {
                bool result = await DeleteLoginsByUser(user.SiteId, user.Id, cancellationToken);
                result = await DeleteClaimsByUser(user.SiteId, user.Id, cancellationToken);
                result = await DeleteUserRoles(user.UserId, cancellationToken);
            }
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            using (DbDataReader reader = await dbSiteUser.GetSingleUser(userId, cancellationToken))
            {
                if (reader.Read())
                {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (DbDataReader reader = await dbSiteUser.GetSingleUser(userGuid, cancellationToken))
            {
                if (reader.Read())
                {
                    SiteUser user = new SiteUser();
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            using (DbDataReader reader = await dbSiteUser.GetSingleUser(siteId, email, cancellationToken))
            {
                if (reader.Read())
                {
                    SiteUser user = new SiteUser();
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (DbDataReader reader = await dbSiteUser.GetCrossSiteUserListByEmail(email, cancellationToken))
            {
                while (reader.Read())
                {
                    UserInfo user = new UserInfo();
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            bool found = false;
            cancellationToken.ThrowIfCancellationRequested();
            using (DbDataReader r = await dbSiteUser.GetSingleUser(siteId, email, cancellationToken))
            {
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (DbDataReader reader = await dbRoles.GetByName(siteId, roleName, cancellationToken))
            {
                while (reader.Read())
                {
                    role = new SiteRole();
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (DbDataReader reader = await dbSiteUser.GetRolesByUser(siteId, userId, cancellationToken))
            {
                while (reader.Read())
                {
        ...
        {
            cancellationToken.ThrowIfCancellationRequested();
            return await dbRoles.GetCountOfUsersNotInRole(siteId, roleId, searchInput, cancellationToken);
        }
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            using (DbDataReader reader = await dbUserLogins.GetByUser(siteId, userId, cancellationToken))
            {
                while (reader.Read())
                {
                    UserLogin userLogin = new UserLogin();
