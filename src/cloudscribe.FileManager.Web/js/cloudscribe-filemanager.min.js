(function () {
    var fileManager = {
        ...
        uploadUrl: $("#fmconfig").data("upload-url"), URL: window.URL || window.webkitURL, console: window.console || { log: function () { } }, image: $('#image'), saveLocalButton: $('#btnSaveLocal'), uploadCropButton: $('#btnUploadCrop'), croppedFileName: $('#croppedFileName'), chkConstrainCrop: $('#chkContrainWidthOfCrop'), cropMaxWidthInput: $('#cropMaxWidth'), dataX: $('#dataX'), dataY: $('#dataY'), dataHeight: $('#dataHeight'), dataWidth: $('#dataWidth'), dataRotate: $('#dataRotate'), dataScaleX: $('#dataScaleX'), dataScaleY: $('#dataScaleY'), outputHeight: $('#dataNewHeight'), outputWidth: $('#dataNewWidth'), setup: function () {
            ...
            }; var originalImageURL = cropManager.image.attr('src'); var uploadedImageURL; $('[data-bs-toggle="tooltip"]').tooltip(); cropManager.image.on({ ready: function (e) { }, cropstart: function (e) { }, cropmove: function (e) { }, cropend: function (e) { }, crop: function (e) { }, zoom: function (e) { } }).cropper(options); if (!$.isFunction(document.createElement('canvas').getContext)) { cropManager.uploadCropButton.prop('disabled', !0) }
            ...
            $('.docs-toggles').on('change', 'input', function () {
                ...
                if (cropManager.image.data('cropper') && data.method) {
                    ...
                    switch (data.method) {
                        case 'scaleX': case 'scaleY': $(this).data('option', -data.option); break; case 'getCroppedCanvas': if (result) { $('#getCroppedCanvasModal').modal().find('.modal-body').html(result); $('#getCroppedCanvasModal').modal('show'); if (!cropManager.saveLocalButton.hasClass('disabled')) { cropManager.saveLocalButton.attr('href', result.toDataURL('image/jpeg')) } }
